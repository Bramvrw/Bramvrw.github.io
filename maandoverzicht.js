...
// Deze functie haalt data over de verschillende opnametypes 
function OpnamesDagType() {
  ...
  request3.onload = function() {
    ...
    request4.onload = function() {
      ...
      let MedischGemiddeldArrayNieuw = [];
      for (let i = 0; i < MedischArray2021.length; i++) {
          MedischGemiddeldArrayNieuw[i] = (MedischArray2021[i] + MedischArray2022[i]) / 2;
      }

      let SpoedGemiddeldArrayNieuw = [];
      for (let i = 0; i < SpoedArray2021.length; i++) {
        SpoedGemiddeldArrayNieuw[i] = (SpoedArray2021[i] + SpoedArray2022[i]) / 2;
      }

      let GeplandGemiddeldArrayNieuw = [];
      for (let i = 0; i < GeplandArray2021.length; i++) {
          GeplandGemiddeldArrayNieuw[i] = (GeplandArray2021[i] + GeplandArray2022[i]) / 2;
      }

      // Call your function to create the graph
      maakMaandGrafiekOpnametypes(MedischArray, SpoedArray, GeplandArray, MedischGemiddeldArrayNieuw, SpoedGemiddeldArrayNieuw, GeplandGemiddeldArrayNieuw);
    };
  };
};
